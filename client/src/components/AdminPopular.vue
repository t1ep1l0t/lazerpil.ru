<template>
  <section class="popular">
    <div class="popular__head">
      <h2 class="popular__title">Поплуряное</h2>
    </div>
    <div class="popular__add">
      <span class="popular__subtitle">Добавить новый элемент в Популярное</span>
      <div class="popular__add__form">
        <div class="form">
          <input type="file" name="" id="file" ref="file" @change="get_picture">
          ...
          <v-text-field class="input"
              ...
              v-model="price"
          ></v-text-field>
          <button class="button" @click="add_new_popular">отправить</button>
        </div>
      </div>
    </div>
    <div class="popular__actual">
      <div class="popular__actual__head">
        <span class="popular__subtitle">Все актуальные поплуряные</span>
        <button class="popular__actual__button"
                v-if="$store.state.populars && $store.state.populars.length > 0"
                @click="delegate_all_popular"
        >
          Удалить все актуальные
      ...
      <ul class="popular__list" v-if="$store.state.populars">
        ...
        >
          ...
          <button class="popular__delete" @click="delete_one_popular(item)">
      ...
      <span v-if="!$store.state.populars || !$store.state.populars.length > 0">Популярного нет</span>
    </div>
  </section>
</template>
